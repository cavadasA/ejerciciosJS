...
window.onload = function () {
    ...
    body.addEventListener("click", event => {
        if (event.target.classList.contains("menorDeEdad") || event.target.classList.contains("mayorDeEdad")) {
            let datos = event.target.innerHTML;
            let modalBody = document.getElementsByClassName("modal-body");
            event.target.setAttribute("data-toggle", "modal");
            event.target.setAttribute("data-target", "#datos")
            modalBody[0].innerHTML = datos;
            let botonParaEliminar = document.getElementById("eliminarPersona");
